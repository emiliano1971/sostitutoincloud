...
-- Nessuna FK è ON DELETE CASCADE verso booking: l'ordine va rispettato a mano.
-- È lo stesso ordine di BookingService.deleteWithCascade().
--
...
UNION ALL
SELECT 'cu_record (indiretto)', cr.id::text, cr.stato
FROM cu_record cr
...
SELECT 'f24_record (indiretto)' AS tabella, f.id::text AS id, f.stato AS dettaglio
FROM f24_record f
JOIN withholding_ledger wl ON wl.fk_f24_record_id = f.id
WHERE wl.fk_booking_id = :booking_id
  AND f.stato IN ('sent', 'paid')
UNION ALL
SELECT 'settlement (indiretto)', s.id::text, s.stato
FROM settlement s
