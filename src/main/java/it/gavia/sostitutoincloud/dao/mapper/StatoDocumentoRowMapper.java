package it.gavia.sostitutoincloud.dao.mapper;

import it.gavia.sostitutoincloud.model.StatoDocumento;
import lombok.extern.log4j.Log4j2;
import org.springframework.jdbc.core.RowMapper;

import java.sql.ResultSet;
import java.sql.SQLException;
import java.time.LocalDateTime;

@Log4j2
public class StatoDocumentoRowMapper implements RowMapper<StatoDocumento> {

    @Override
    public StatoDocumento mapRow(ResultSet rs, int rowNum) throws SQLException {
        return StatoDocumento.builder()
                .id(rs.getInt("id"))
                .codice(rs.getString("codice"))
                .descrizione(rs.getString("descrizione"))
                .isError(rs.getBoolean("is_error"))
                .finale(rs.getBoolean("finale"))
                .attivo(rs.getBoolean("attivo"))
                .createdAt(rs.getObject("created_at", LocalDateTime.class))
                .updatedAt(rs.getObject("updated_at", LocalDateTime.class))
                .build();
    }
}
